# E1/E2 Extension Results

This report summarizes the completed E1 magnitude sweeps and the E2 Companion A
rescoring analysis. Delta convention is unchanged: `metric(comparator) -
metric(RL)`. Positive delta means the learned/RL method has the lower metric;
negative delta means the comparator has the lower metric.

## Artifact Index

| Component | Status | MLflow run | Primary table | Figure |
|---|---|---|---|---|
| E1 DeepRM | complete with Table-IV-compatible harness; `epsilon=0.2` reported as noncompletion | `2d4588396f474862bbf61bfebbf16ba4` | `results/paper/experiments/e1_magnitude_sweep/deeprm/tables/e1_deeprm_magnitude_sweep.csv` | `results/paper/experiments/e1_magnitude_sweep/deeprm/figures/e1_deeprm_magnitude_sweep.pdf` |
| E1 Decima | complete | `587abbac142e4d83aecd3924271d8909` | `results/paper/experiments/e1_magnitude_sweep/decima/tables/e1_decima_magnitude_sweep.csv` | `results/paper/experiments/e1_magnitude_sweep/decima/figures/e1_decima_magnitude_sweep_inset.pdf` |
...
- DeepRM E1 uses the locked Table IV checkpoint `policy_final.pt`, the locked
  `Tetris*` comparator (`TetrisScheduler(source_dot=True)`), SJF as the
  secondary comparator, and the same policy/statistic seed offsets as
...
- Decima P1 is not sign-stable inside the realistic lambda<=1 regime: lambda=0.25 is negative (-13,305.0, Holm p(Delta<0)=0.00264) while lambda=0.5 and 1.0 are strongly positive. The lag-injection check found no fractional-step rounding in the path: delays are continuous floats added directly to task `finish_time` and `duration`. The deterministic exponential draw includes the lag mean in its hash key, so adjacent lambda cells are valid per-cell paired tests but not common-random-number scaled versions of the same delay draws. This weakens any simple monotone “Decima P1 falsified across magnitudes” claim.
...
- Decima P1 requires a magnitude-dependent interpretation because lambda=0.25
  reverses sign while lambda=0.5 and 1.0 are positive.
- DeepRM E1 remains positive through the realistic P1/P3 range; epsilon=0.2 is
  reported as noncompletion rather than as a finite paired delta.
- Rossi E1 separates the bundled-threshold lag collapse from the HPA-v2
  attenuation result: bundled-threshold cost grows for k>=5, while HPA-v2
  remains far below the bundled collapse across the lag grid.
- E2 Companion A is a deterministic rescoring sensitivity, not full
  native-objective policy evidence.
